...
def _paper_to_dict(paper: ArxivPaper) -> Dict[str, object]:
    """Convert an `ArxivPaper` dataclass to a JSON-serializable dictionary.

    Args:
        paper: The `ArxivPaper` instance to convert.

    Returns:
        Dictionary with stable keys appropriate for agent tool responses.
    """
    return {
        "id": paper.id,
        "title": paper.title,
...
def arxiv_search_tool(
    ...
    Args:
        query: Free-text arXiv search query.
        max_results: Maximum number of results to return.
        start: Pagination start index (0-based) within the result stream.
        categories: Optional list of arXiv category filters (e.g., ["cs.AI", "cs.LG"]).
        date_from_days: If provided, limit results to those submitted within the last N days.

    Returns:
        List of dictionaries; each dictionary represents a paper with keys like
        `id`, `title`, `authors`, `summary`, `categories`, `published`, `updated`,
        `pdf_url`, and `abs_url`.
    """
    parser = ArxivParser()
...
def arxiv_get_paper_tool(arxiv_id: str) -> Optional[Dict[str, object]]:
    ...
    Args:
        arxiv_id: The arXiv identifier (with or without the "arXiv:" prefix; version suffix allowed).

    Returns:
        A dictionary representation of the paper if found; otherwise None.
    """
    parser = ArxivParser()
